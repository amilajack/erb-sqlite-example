...
const establishConnection = async (conn, maxAttempts, attemptDelay) => {
  ...
  do {
    try {
      conn.logger(
        `[db:${conn.name}] Connection attempt ${attempts + 1}/${maxAttempts}`
      );
    ...
    (model) =>
      ...
      new Promise(async (resolve, reject) => {
        ...
        try {
          ...
          await model.start(
            conn,
            connectionHandler.getModel(model.name, conn.name)
          );
          resolve();
        } catch (err) {
...
const start = async (settings) => {
  ...
  try {
    ...
    throw new Error(
      `Error - Sync Model (initModels) - [db:${name}] model: ${err.model.name}, ${err.err.message}`
    );
  }
  ...
  try {
    ...
    throw new Error(
      `Error - Sync Model (startModels) - [db:${name}] model (start): ${err.model.name}, ${err.err.message}`
    );
  }
};
